...
export default class SignUpPage extends BasePage {
  ...
  static choosePlan() {
    cy.fixture("signUp").then((data) => {
      ...
      cy.get(".wizard__subtitle").should("have.text", data.plan);
    });
  }

  static disabledStartWatchingButton() {
    cy.contains("Start watching").should("have.class", "-disabled");
  }
  ...
  static completeSignUp() {
    cy.fixture("signUp").then((data) => {
      cy.get('form[name="form-sign-up"]').within(() => {
        ...
        if (data.gender == "Female") cy.get('input[label="Female"]').check();
        else cy.get('input[label="Male"]').check();
      ...
      if (data.payment == "0") {
        ...
        cy.get('span[data-hosted-id="encryptedCardNumber"]').within(() => {
          cy.get("iframe.js-iframe").should(
            ($iframe) =>
              expect(
                $iframe.contents().find('input[placeholder="Card Number"]')
              ).to.exist
          );
        ...
        cy.get('span[data-hosted-id="encryptedExpiryDate"]').within(() => {
          cy.get("iframe.js-iframe")
            ...
            .then(($iframe) => {
              const $body = $iframe.contents().find("body");
              cy.wrap($body).find('input[placeholder="MM/YY"]').type("1222");
            });
        });
        cy.get('span[data-hosted-id="encryptedSecurityCode"]').within(() => {
          cy.get("iframe.js-iframe")
            .should(
              ($iframe) =>
                expect($iframe.contents().find('input[placeholder="CVV"]')).to
                  .exist
            )
            .then(($iframe) => {
              const $body = $iframe.contents().find("body");
              cy.wrap($body).find('input[placeholder="CVV"]').type("122");
            });
        });
      } else
